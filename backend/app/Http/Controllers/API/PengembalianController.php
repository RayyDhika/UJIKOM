...
use App\Http\Request\Pengembalian\StorePengembalianRequest;
use App\Http\Request\Pengembalian\UpdatePengembalianRequest;
use App\Models\Alat;
...
class PengembalianController extends Controller
{
    ...
    public function index() : JsonResponse
    {
        ...
        'peminjaman.detailPinjam.alat', 'petugas']);
    ...
    public function store(StorePengembalianRequest $request): JsonResponse
    {
        try {
            $pengembalian = DB::transaction(function () use ($request) {
                ...
                // 2. ubah status di tabel peminjaman utamma
                ...
                foreach ($peminjaman->detailPinjam as $detail) {
                    ...
                    //increment() otomstis menambah nilai pada field yang ditentukan
                    $alat->increment('stok', $detail->jumlah);
                }

                //optional catat ke log aktivitas petugas
                auth()->user()->logAktivitas()->create([
                    'aktivitas' => "Memproses pengembalian peminjaman ID:
                    # {$peminjaman->id} dengan status akhir: {$statusPeminjamanBaru}."
                ]);

                //load relasi agar response jsoan lebih informatif
            ...
            //tabfkap pesan error dari throw exception diatas (misal status bukan 'dipinjam)
    ...
    {
        //mengamannkan data dengan membatasi field yang boleh dikoreksi petugas
        $pengembalian->update([
            'kondisi_kembali' => $request->kondisi_kembali,
    ...
    public function destroy(Pengembalian $pengembalian) : JsonResponse
    {
        try {
            DB::transaction(function () use ($pengembalian) {
                ...
                //tarik kembali stok ke gudang (karena status kembali di batalkan, stok berkurang lagi)
                foreach ($peminjaman->detailPinjam as $detail) {
                ...
                //log aktivitas jika ,etode/relasi tersedia 
